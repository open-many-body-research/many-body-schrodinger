import FiniteDefectKernel_v1
import SpectralBottom_v2
import Mathlib.Analysis.SpecificLimits.Basic

/-! Spectral points below a coercive finite-dimensional comparison are actual
...
theorem selfAdjoint_spectral_eigenvector_of_finite_defect
    (m : ℕ) (A : E →ₗ.[ℂ] E) (hA : IsSelfAdjoint A)
    (l : E →L[ℂ] (Fin m → ℂ))
    (β C : ℝ) (hC : 0 ≤ C)
  ...
  obtain ⟨u,hu,hSu⟩ := closed_operator_kernel_of_finite_defect m S
    (operatorShift_isClosed A hA.isClosed (r : ℂ)) l (sub_pos.mpr hrβ) hC hsform x hx hres0
  refine ⟨u,hu,?_⟩
  exact sub_eq_zero.mp hSu

#print axioms selfAdjoint_spectral_eigenvector_of_finite_defect
