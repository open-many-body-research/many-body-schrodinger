import CompactDefectCauchy_v1

/-! A rank-one defect in coercivity forces attainment of an approximate
...
theorem closed_operator_kernel_of_scalar_defect
    (A : E →ₗ.[ℂ] E) (hA : A.IsClosed) (l : E →L[ℂ] ℂ)
    {b C : ℝ} (hb : 0 < b) (hC : 0 ≤ C)
  ...
  have hlbounded : ∀ n, l (x n : E) ∈ Metric.closedBall (0 : ℂ) ‖l‖ := by
    intro n
    rw [Metric.mem_closedBall,dist_zero_right]
    exact (l.le_opNorm (x n : E)).trans_eq (by rw [hx n,mul_one])
  obtain ⟨c,hc,φ,hφ,hconv⟩ := (isCompact_closedBall (0 : ℂ) ‖l‖).tendsto_subseq hlbounded
  ...
  have hyC := approximate_null_cauchy_of_scalar_cauchy A l hb hC hform y hy hAy hly
...
#print axioms closed_operator_kernel_of_scalar_defect
