...
theorem compact_exists_smooth_cutoff {N : ℕ} {K Ω : Set (Configuration N)}
  ...
  classical
  have hx (x : K) := open_exists_smooth_cutoff_at hΩ (hKO x.property)
  choose r hr b hb hc hs h1 using hx
  obtain ⟨s,hcover⟩ := hK.elim_finite_subcover
    (fun x : K => Metric.ball (x : Configuration N) (r x)) (fun _ => Metric.isOpen_ball)
    (fun x hx => Set.mem_iUnion.mpr ⟨⟨x,hx⟩,by simpa using hr ⟨x,hx⟩⟩)
  let η : Configuration N → ℝ := fun y => 1-∏ x ∈ s,(1-b x y)
...
#print axioms compact_exists_smooth_cutoff
