...
| Claim | Result | Tier |
|---|---|---|
| S2-001 | **Abstract continuum Temple.** Suppose $`\beta\Vert x\Vert ^2 \le \operatorname{Re}\langle x,Hx\rangle + C\vert \ell(x)\vert ^2`$ on $`D(H)`$, and a unit $`\psi \in D(H)`$ has mean $`L \le \mu \le U < \beta`$ and $`\Vert H\psi-\mu\psi\Vert ^2 \le r`$. Then $`\inf\sigma(H) \in [L - r/(\beta-U),\, U]`$, and the ground state is attained and simple. | L (the form comparison is a *premise*) |
