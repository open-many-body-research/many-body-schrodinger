...
GitHub Actions is disabled. Our [local verification runner](docs/local-verification.md) runs these checks on maintainer-approved commits and reports the `local/verify` GitHub status. Run `python3 tools/verify_local.py --trusted-checkout --mode full` from a clean committed checkout.
...
2. **Bring evidence that matches your tier:**
   ...
   - **C:** an exact-arithmetic certificate with a separate checker run by the local verifier.
